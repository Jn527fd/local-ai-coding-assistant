...
export default function App() {
  ...
  const discardAndCloseContextModal = () => {
    setDraftSystemPrompt(savedSystemPrompt)
    if (hasUnsavedPrompt) setPromptFileName(null)
    setPromptFileError("")
  ...
  const saveSystemPrompt = async () => {
    ...
    try {
      ...
      setPromptFileError(normalized.message)
    }
  }
  ...
  const importModelFile = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > 1024 * 1024) {
      setPromptFileError("Choose a UTF-8 system prompt text file smaller than 1 MB.")
      return
    }
    ...
    try {
      ...
      if (!content.trim()) {
        setPromptFileError("That system prompt file is empty.")
        return
      }
      ...
      setPromptFileError(
        "Choose a valid UTF-8 system prompt text file smaller than 1 MB.",
      )
    }
  }
