...
interface ProposedConversationMetadata {
  systemPrompt?: string
  sourceIds?: string[]
  temporary?: boolean
}
...
export function mapBackendConversationToConversation(
  ...
  const metadata = readProposedMetadata(record.metadata)
...
function readProposedMetadata(
  metadata: Record<string, unknown> | undefined,
): ProposedConversationMetadata {
  const proposed = metadata?.proposedFrontend
