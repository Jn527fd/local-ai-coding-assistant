...
export function SystemPromptModal({
  ...
  modelFileRef: RefObject<HTMLInputElement | null>
  importModelFile: (event: ChangeEvent<HTMLInputElement>) => void
  modelFileName: string | null
  setModelFileName: Dispatch<SetStateAction<string | null>>
  modelFileError: string
  setModelFileError: Dispatch<SetStateAction<string>>
      ...
      >
        ...
        <div className="context-prompt-controls">
          ...
          <input
            ...
            className="context-modelfile-input"
            aria-label="Choose a Modelfile to import"
          />
          <button type="button" onClick={() => modelFileRef.current?.click()}>
            <span aria-hidden="true">↑</span> Upload Modelfile
          </button>
        </div>
        <p className="context-modelfile-help">
          Write instructions below, or upload a text Modelfile to fill the
          editor.
        </p>
        {modelFileName && (
          <div className="context-modelfile-status" role="status">
            <span aria-hidden="true">✓</span>
            <span title={modelFileName}>{modelFileName}</span>
            <button
              type="button"
              onClick={() => {
                setDraftSystemPrompt("")
              ...
              aria-label={`Remove imported ${modelFileName}`}
            >
              Remove
            </button>
          </div>
        ...
        {modelFileError && (
          <p className="context-modelfile-error" role="alert">
            {modelFileError}
          </p>
        )}
        <textarea
